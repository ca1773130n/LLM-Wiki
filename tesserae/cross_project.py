"""Cross-project link resolution for Tesserae's multi-vault registry.

Bet **B2** in the competitive-positioning research: the registry
(``~/.tesserae/registry.json``) is a *graph substrate*, not just a CLI
...
See also:
    ...
    * :class:`tesserae.mcp_server.ProjectRegistry` — the file-backed
...
# ``.tesserae/site/<kind>/<slug>.html``. We don't restrict the regex to
...
def cross_project_resolve(uri: str, *, registry: Optional[ProjectRegistry] = None) -> CrossProjectRef:
    ...
    try:
        ...
        root = Path(root_str)
    # Canonical static-site layout: ``<root>/.tesserae/site/<kind>/<slug>.html``.
    page = root / ".tesserae" / "site" / kind / f"{slug}.html"
...
def find_wiki_uris_in_text(body: str) -> list[str]:
    ...
    Used at compile time by :func:`tesserae.site.pages.build_graph_payload`
