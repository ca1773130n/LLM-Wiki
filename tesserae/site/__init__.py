"""``tesserae.site`` package.

This module is the real ``StaticSiteBuilder`` for the redesigned Tesserae:
...
class StaticSiteBuilder:
    ...
    # by ``graph_view.show_sources`` in ``.tesserae/config.json``;
    # :meth:`tesserae.project.ProjectWiki.build_site` reads the flag and
    # forwards it here.
    show_sources: bool = False
    # Optional GitHub blob URL prefix (e.g.
    # ``"https://github.com/ca1773130n/Tesserae/blob/main"``). When set,
    ...
    # 404ing. Computed in :meth:`tesserae.project.ProjectWiki.build_site`
    # from ``site.github_repo_url`` / ``site.github_blob_base`` in
    # ``.tesserae/config.json``. ``None`` → no rewriting (legacy behavior).
    ...
    def write_site(
        ...
        signature still used by ``tesserae.project.ProjectWiki.build_site``):
        ...
        # lives at the project-wiki root (``.tesserae/.build-history.jsonl``),
        ...
        # ``tesserae.site.pages._detail_page`` when wiring the
...
_NGINX_SNIPPET = """# nginx snippet for serving the Tesserae static site with pre-compressed assets.
