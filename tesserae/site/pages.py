"""Page-template renderers for the Tesserae static site.
...
``ai_siblings_footer``, ``toc``) come from :mod:`tesserae.site.components` —
Subagent D owns those primitives. ``top_related`` comes from
:mod:`tesserae.site.relevance`. Both are imported eagerly; missing modules
...
class SiteContext:
    ...
    # Set via the ``graph_view.show_sources`` knob in ``.tesserae/config.json``.
    ...
    # list = widget falls back to the original "host with tesserae serve"
    # one-liner in degraded mode. Populated by :meth:`build` from
    # ``.tesserae/external/raganything/qa-cache.json`` (CI-restored) or
...
# ``.tesserae/config.json`` (default is hide). ``SiteContext.show_sources``
...
def build_graph_payload(ctx: SiteContext) -> Dict[str, object]:
    ...
    # in ``.tesserae/config.json`` to restore the dense view.
...
def render_graph_view(ctx: SiteContext) -> str:
    ...
    if not ctx.show_sources:
        legend_items += (
            ...
            'title="Toggle via graph_view.show_sources in .tesserae/config.json">'
