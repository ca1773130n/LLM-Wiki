"""Raw-document viewer route for the Tesserae site.
...
def derive_project_root(wiki_root: Optional[Path]) -> Optional[Path]:
    ...
    Convention: ``<project_root>/.tesserae/wiki/`` — see ``ProjectPaths``.
    ...
    if wiki_root is None:
        return None
    p = Path(wiki_root)
    # parent[0] == .tesserae, parent[1] == project root.
    if p.name == "wiki" and p.parent.name == ".tesserae":
...
def build_wiki_link_resolver(
    ...
    def resolve(kind: str, key: str) -> Optional[str]:
        if not kind or not key:
            return None
        if kind == "papers":
            # ``papers/<arxiv>/paper.md`` → the Tesserae paper detail page.
...
def _render_markdown_body(
    ...
    def _link_rewriter(target: str) -> str:
        ...
        # Project-file references in docs (``[research_graph.py](../tesserae/research_graph.py)``)
        ...
        # Priority order:
        #   (1) ../papers/<slug>.html — the Tesserae paper page is where
