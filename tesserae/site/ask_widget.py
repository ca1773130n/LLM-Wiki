...
* **Live mode** — when ``/api/ask/health`` responds OK (``tesserae project
...
def _load_demo_qa(project_root: _Path | str | None) -> list[dict]:
    ...
    Looked up in this order:
      1. ``<project_root>/.tesserae/external/raganything/qa-cache.json``
    ...
    candidates = [
        root / ".tesserae" / "external" / "raganything" / "qa-cache.json",
