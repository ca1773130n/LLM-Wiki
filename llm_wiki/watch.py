...
``.llm-wiki/.watch-cache.json`` so the watcher can be driven from cron.
"""
...
class WatchLoop:
    ...
    ``sources`` in ``.llm-wiki/config.json``. Non-``.md``/``.markdown`` files
    ...
    def __init__(
        ...
        *,
        ...
        self._cycles = 0
        self._cache_path = self.project_root / ".llm-wiki" / self.CACHE_FILENAME
    ...
    def _resolve_watch_paths(self, watch_paths: Sequence[str | Path] | None) -> List[Path]:
        ...
        if watch_paths:
            ...
            cfg_path = self.project_root / ".llm-wiki" / "config.json"
            if cfg_path.exists():
                try:
    ...
    def snapshot(self) -> dict[Path, tuple[float, int]]:
        ...
        for root in self.watch_paths:
            ...
            for dirpath, dirnames, filenames in os.walk(root):
                # Skip dotfile dirs (e.g. ``.git``, ``.llm-wiki``)
