...
(``[wiki_store.py](../llm_wiki/wiki_store.py)``), the rendered HTML preserves
...
in ``.llm-wiki/config.json`` (e.g. ``"https://github.com/ca1773130n/LLM-Wiki"``).
"""
...
def resolve_to_repo_relative(
    ...
    For example, ``[x](../llm_wiki/x.py)`` from ``docs/feature-map.md``
    resolves to ``llm_wiki/x.py``, while ``[x](../../llm_wiki/x.py)`` from
    ``docs/i18n/feature-map.zh.md`` resolves to the same ``llm_wiki/x.py``.
    """
    if not href:
        return None
    s = href.split("#", 1)[0].split("?", 1)[0].strip()
