...
- **Claude Code CLI** 使用 OAuth。把 RAG-Anything 运行时查询切换到 Claude，请设置 `--raganything-llm-provider claude`。多账号场景使用 `--raganything-claude-config-dir ~/.claude-personal2`（Tesserae 会在每次调用前导出 `CLAUDE_CONFIG_DIR`）。
