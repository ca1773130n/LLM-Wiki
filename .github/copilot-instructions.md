...
- **brownfield**: Scan a root directory for existing git repositories and linked worktrees, then manage default repos used as context in i
- **cancel**: Cancel stuck or orphaned executions by session ID, cancel all running sessions, or interactively pick from active execut
- **evaluate**: Evaluate an execution session using the three-stage verification pipeline.
- **evolve**: Start, monitor, or rewind an evolutionary development loop. The loop iteratively
- **help**: Full reference guide for Ouroboros power users.
- **interview**: Socratic interview to crystallize vague requirements into clear specifications.
- **pm**: PM-focused Socratic interview that produces a Product Requirements Document.
- **publish**: Convert a Seed specification into structured GitHub Issues for team workflows.
- **qa**: Standalone quality assessment for any artifact — code, documents, API responses, test output, or custom content. Unlike 
- **ralph**: Persistent self-referential loop until verification passes. "The boulder never stops."
- **release**: Cut a release of Tesserae. NEVER skip a step; NEVER --no-verify; NEVER force-push.
- **resume-session**: Recover in-flight Ouroboros sessions after an unexpected MCP server disconnect.
- **run**: Execute a Seed specification through the Ouroboros workflow engine.
- **seed**: Generate validated Seed specifications from interview results.
- **setup**: Guided onboarding wizard that converts users into power users.
- **status**: Check session status and measure goal drift.
- **tutorial**: Interactive tutorial that teaches Ouroboros through hands-on experience.
- **ui-verify**: Triggered whenever a user request involves "make X look like Y", "move Z next
- **unstuck**: Break through stagnation with lateral thinking personas.
- **update**: Check for updates and upgrade Ouroboros (PyPI package + runtime integration).
- **welcome**: Interactive onboarding for new Ouroboros users.
