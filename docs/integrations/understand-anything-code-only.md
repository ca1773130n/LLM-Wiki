...
This is a follow-up to [understand-anything.md](understand-anything.md). The base doc explains how to install and enable [Understand-Anything](https://github.com/Lum1104/Understand-Anything) (UA) as a companion that produces a code-graph in `.understand-anything/knowledge-graph.json`. **This doc explains how to make UA contribute ONLY a code graph and never pollute LLM-Wiki's research-graph Concept layer with section headings extracted from your docs.**
...
### Step 1 — stop the LLM-Wiki side from importing UA as Concepts

Edit `.llm-wiki/config.json` and set both `enabled: false` and `sync_mode: "disabled"` on the UA tool entry. Both belt-and-suspenders flags are checked by the merge code path:
...
LLM-Wiki regenerates `.llm-wiki/external/understand-anything.md` only when the tool is enabled, so deleting it is safe once Step 1 is in place.
...
1. **Scope UA itself to code, not docs.** UA accepts include/exclude patterns; configure it to walk only `src/`, `lib/`, `llm_wiki/`, etc. and explicitly exclude `docs/`, `README*.md`, and `docs/i18n/`. The exact config knob lives in UA's own docs at [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything).
2. **Re-enable in `.llm-wiki/config.json`**: flip `enabled` back to `true`, `sync_mode` back to `"native_graph"`, `auto_refresh` back to `true`.
...
The asymmetry stings — disabling is one config flip, re-enabling cleanly requires understanding UA's source-scoping — but it's the right boundary. UA's job is code graphs, LLM-Wiki's job is research graphs, and the seam between them should never let doc headings cross from one side to the other.

## Where this fits

| Layer | Concern | Configured via |
|---|---|---|
| UA's own walker | What files UA reads in the first place | UA's config (out of scope for LLM-Wiki) |
| `auto_refresh` on UA tool | Whether `llm_wiki project compile` re-runs UA | `.llm-wiki/config.json` external_tools entry |
| `enabled` on UA tool | Whether LLM-Wiki considers UA at all | `.llm-wiki/config.json` external_tools entry |
| `sync_mode` on UA tool | Whether UA's nodes get merged into the research graph | `.llm-wiki/config.json` external_tools entry |
