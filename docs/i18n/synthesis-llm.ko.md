...
<!-- translations:end -->
Tesserae에는 두 가지 합성 경로가 있습니다. 기본값은 네트워크를 전혀 호출하지 않는 결정적 휴리스틱입니다. 연구 그래프에서 예측 가능하고 멱등적인 Markdown 템플릿을 생성합니다. 선택 사항인 **LLM 업그레이드 경로**는 다른 모든 불변성(멱등성, citation 추적, hash-stable 본문)을 그대로 유지하면서, 매 compile마다 Claude가 작성한 prose로 해당 템플릿을 대체합니다.
...
전체 블록은 약 500 token입니다. canonical text는 [`tesserae/llm_synthesis.py`](../../tesserae/llm_synthesis.py)를 참고하세요. 그곳의 byte가 하나라도 바뀌면 실행 중 이후 모든 page의 prompt cache가 무효화되므로, rule text는 의도적으로 고정되어 있습니다.
...
1. `TESSERAE_SYNTHESIS_LLM`이 `1`/`true`/`yes`/`on`입니다.
2. `ANTHROPIC_API_KEY`가 비어 있지 않습니다(또는 프로젝트의 `.tesserae/config.json`에서 `synthesis.api_key`를 설정했습니다).
3. `anthropic` package를 import할 수 있습니다.

이 중 하나라도 누락되면 stderr에 정보성 line 하나를 기록하고(`[tesserae] LLM synthesis disabled (...)`) 휴리스틱으로 fallback합니다.
...
Haiku 4.5(`$1/M` input, `$5/M` output)로 전환하면 동일한 compile 비용은 대략 `~$0.027`입니다. token을 쓰기 전에 prompt 형태를 확인하려면 먼저 `TESSERAE_SYNTHESIS_DRY_RUN=1`로 실행하세요.
...
`.tesserae/wiki/syntheses/`의 append-only `.history.jsonl` ledger는 모든 rewrite의 generator label을 기록하므로, page가 언제 휴리스틱에서 LLM으로(또는 다시) 전환되었는지 audit할 수 있습니다.
