...
<!-- translations:end -->
Ce document résume les fonctionnalités actuellement implémentées dans LLM-Wiki, avec leur état, leurs fichiers source et l’endroit où elles sont documentées.
...
|---|---|---|---|
| `WikiPageStore` (écritures body-hash idempotentes, parseur frontmatter) | ✅ | [`llm_wiki/wiki_store.py`](../../llm_wiki/wiki_store.py) | [architecture.md § Carte des modules](architecture.fr.md#wiki--synthesis-l2) |
| `WikiLayerProjector` — une page md par nœud de couche wiki | ✅ | [`llm_wiki/wiki_projector.py`](../../llm_wiki/wiki_projector.py) | [architecture.md § Pipeline](architecture.fr.md#pipeline) |
...
| Pages `syntheses/` | ✅ | [`llm_wiki/synthesis.py`](../../llm_wiki/synthesis.py) | [frontend-redesign.md § Syntheses](frontend-redesign.fr.md#syntheses) |
...
| Résumés améliorés par LLM (activés par variable d’environnement) | ⚠ | hook uniquement | La base heuristique est livrée ; le hook `LLM_WIKI_SYNTHESIS_LLM=1` reste un stub. |

### Routes du site statique

| Route | État | Source | Notes |
|---|---|---|---|
| `/` (accueil, hero pulse) | ✅ | [`llm_wiki/site/pages.py`](../../llm_wiki/site/pages.py) `render_home` | Ligne de statistiques + points d’entrée sélectionnés + activité récente. |
...
|---|---|---|---|
| Fichier frère `<page>.txt` par page | ✅ | [`llm_wiki/site/exports.py`](../../llm_wiki/site/exports.py) `write_siblings` | Vue texte brut d’une page (sans navigation ni style). |
...
| `search-index.json` | ✅ | [`llm_wiki/site/search.py`](../../llm_wiki/site/search.py) | Palette + recherche de pages ; types de couche wiki uniquement. |
...
|---|---|---|---|
| Tokens de design (thèmes clair + sombre, accent terre cuite) | ✅ | [`llm_wiki/site/tokens.py`](../../llm_wiki/site/tokens.py) | Un bundle CSS dans `assets/style.css`. |
| Bascule de thème (persistée, sans flash) | ✅ | [`llm_wiki/site/js.py`](../../llm_wiki/site/js.py) | `data-theme="dark"` dans `localStorage`, appliqué avant le rendu. |
...
| Pages d’historique de sessions du harnais | ✅ | [`llm_wiki/harness_sessions.py`](../../llm_wiki/harness_sessions.py) + [`llm_wiki/site/sessions.py`](../../llm_wiki/site/sessions.py) | Import explicite Claude Code/Codex ; index `/sessions/` et pages détail avec tours markdown, rail de tours à gauche, utilisation d’outils repliée et entrées de recherche. |

### Pipeline + CLI

| Fonctionnalité | État | Source | Notes |
|---|---|---|---|
| `project compile` appelle synthesis + wiki + site dans l’ordre | ✅ | [`llm_wiki/project.py`](../../llm_wiki/project.py) | Phase 3 du plan de refonte. |
| `project build-site` autonome | ✅ | `project.py` + [`llm_wiki/cli.py`](../../llm_wiki/cli.py) | Lit `wiki/` + `graph.json`, écrit `site/`. |
| `project serve` HTTP local | ✅ | `cli.py` | Serveur stdlib simple. |
| `project deploy` → GitHub Pages | ✅ | [`llm_wiki/deploy.py`](../../llm_wiki/deploy.py) | Push de worktree vers `gh-pages` ; `--enable-pages` optionnel via CLI `gh`. `--build`, `--dry-run`, `--branch`, `--remote`, `--force`. |
| `project sessions discover/import/list` | ✅ | [`llm_wiki/harness_sessions.py`](../../llm_wiki/harness_sessions.py) + `cli.py` | Historique de sessions entrant pour Claude Code/Codex ; découverte explicite et bornée au répertoire de travail du projet. |
| `project watch` rebuild-on-change | ⚠ | [`llm_wiki/cli.py`](../../llm_wiki/cli.py) | Subagent R termine le watcher par polling — l’interface d’arguments `--interval`, `--debounce`, `--once`, `--paths`, `--quiet` est en place ; le corps de boucle de rebuild est livré dans cette passe. |
...
- ✅ Commandes console : `llm_wiki`, `llm-wiki`, `llm_wiki_mcp`.
...
- ✅ `llm_wiki project deploy` (nouveau — GitHub Pages)
- ✅ `llm_wiki project sessions discover/import/list` (import explicite d’historique d’agent local)
- ⚠ `llm_wiki project watch` (en cours)
...
- ✅ `llm_wiki_mcp` / `python3 -m llm_wiki.mcp_server` via stdio JSON-RPC.
