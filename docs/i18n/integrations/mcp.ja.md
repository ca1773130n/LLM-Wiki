...
MCP サーバーは型付きグラフへの**読み取りインタフェース**です。**書き込み経路**（ソースの取り込み、再コンパイル、RAG-Anything や Understand-Anything のような連携ツールの更新）には CLI を直接使ってください。両者は疎結合です: CLI が `.tesserae/` を更新し、MCP サーバーは次の tool 呼び出しでそこにあるものを読み取ります。
