...
* **9 comandos slash** — siete envoltorios 1:1 alrededor de la CLI (`/tesserae:compile`, `/tesserae:ask`, `/tesserae:sessions-import`, `/tesserae:build-site`, `/tesserae:serve`, `/tesserae:obsidian-sync`, `/tesserae:setup`) más dos macros de flujo de trabajo (`/tesserae:refresh` encadena import + compile + vault-sync; `/tesserae:status` muestra conteos del grafo y última compilación).
...
- **Comandos slash** = acciones de flujo de trabajo que invocas explícitamente (compile, refresh, vault-sync). Alto apalancamiento pero debe ser tu decisión.
