...
### Schritt 1 — Verhindere, dass die LLM-Wiki-Seite UA als Concepts importiert

Bearbeite `.llm-wiki/config.json` und setze sowohl `enabled: false` als auch `sync_mode: "disabled"` am UA-Tool-Eintrag. Beide Flags werden vom Merge-Code-Pfad geprüft (doppelte Absicherung):
...
LLM-Wiki regeneriert `.llm-wiki/external/understand-anything.md` nur, wenn das Tool aktiviert ist — das Löschen ist also sicher, sobald Schritt 1 umgesetzt ist.
...
1. **Beschränke UA selbst auf Code, nicht auf Docs.** UA akzeptiert include/exclude-Pattern; konfiguriere es so, dass es nur `src/`, `lib/`, `llm_wiki/` usw. durchläuft und `docs/`, `README*.md` sowie `docs/i18n/` explizit ausschließt. Der genaue Config-Schalter ist in den UA-eigenen Docs unter [Lum1104/Understand-Anything](https://github.com/Lum1104/Understand-Anything) dokumentiert.
2. **Reaktiviere in `.llm-wiki/config.json`**: setze `enabled` zurück auf `true`, `sync_mode` zurück auf `"native_graph"`, `auto_refresh` zurück auf `true`.
...
Die Asymmetrie ärgert — Deaktivieren ist ein Config-Flip, sauberes Reaktivieren erfordert Verständnis von UAs Source-Scoping —, aber sie zieht die richtige Grenze. UAs Aufgabe sind Code-Graphen, LLM-Wikis Aufgabe sind Research-Graphen, und die Naht zwischen beiden sollte Doc-Überschriften niemals von einer Seite zur anderen durchlassen.

## Wo das hineinpasst

| Schicht | Anliegen | Konfiguriert über |
|---|---|---|
| UA-eigener Walker | Welche Dateien UA überhaupt liest | UA-eigene Config (außerhalb des Scopes von LLM-Wiki) |
| `auto_refresh` am UA-Tool | Ob `llm_wiki project compile` UA erneut ausführt | `.llm-wiki/config.json`-external_tools-Eintrag |
| `enabled` am UA-Tool | Ob LLM-Wiki UA überhaupt berücksichtigt | `.llm-wiki/config.json`-external_tools-Eintrag |
| `sync_mode` am UA-Tool | Ob UA-Nodes in den Research-Graphen gemerged werden | `.llm-wiki/config.json`-external_tools-Eintrag |
