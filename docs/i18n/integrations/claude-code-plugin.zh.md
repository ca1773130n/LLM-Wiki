...
* **9 个斜杠命令** —— 7 个 CLI 1:1 包装器(`/tesserae:compile`、`/tesserae:ask`、`/tesserae:sessions-import`、`/tesserae:build-site`、`/tesserae:serve`、`/tesserae:obsidian-sync`、`/tesserae:setup`)+ 两个工作流宏(`/tesserae:refresh` 链式执行 import + compile + vault-sync;`/tesserae:status` 显示图谱计数和上次编译)。
...
- **斜杠命令** = 你明确调用的工作流操作(compile、refresh、vault-sync)。高杠杆但应该由你决定。
