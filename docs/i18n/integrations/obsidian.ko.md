...
컴파일은 진실의 원천인 `.llm-wiki/graph.json`과 `.llm-wiki/markdown_projection/`에 위치한 단순 markdown 투영을 생성합니다. Obsidian export는 이 투영 위에 구축되지만 모든 페이지에 Obsidian 네이티브 보강을 덧붙입니다.
...
여러 LLM-Wiki vault를 등록하여 `wiki://` URI가 그들 사이에서 해결되도록 만드세요:
...
Obsidian vault는 타입 그래프의 **읽기 전용 export**입니다. Obsidian에서의 편집은 `.llm-wiki/graph.json`으로 되돌아 흐르지 않습니다. 새 소스나 수정을 반영하려면:
...
컴파일된 HTML 사이트(`llm_wiki project build-site` → `.llm-wiki/site/`)는 공유용입니다 — GitHub Pages, S3, 모든 정적 호스트에 푸시할 수 있습니다. Obsidian vault는 Dataview와 Obsidian 그래프 뷰로 로컬에서 **읽고 쿼리하기** 위한 것입니다. 둘 다 동일한 그래프에서 투영되므로 결코 어긋나지 않습니다.
