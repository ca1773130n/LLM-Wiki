...
执行 `llm_wiki project compile` 后，站点位于 `.llm-wiki/site/`。要在本地浏览：
...
这些是 L1 原始文档 — `.llm-wiki/config.json` 引用的 `data/`、`docs/` 和项目树中的文件。每个 source 都会成为一个 `SourceDocument`（或 `Paper` / `Repository`）节点，并由 `WikiLayerProjector` 投影为 wiki 页面。
...
单个 source 文档，通过 stdlib markdown 管线（`llm_wiki/site/markdown.py`）渲染。正文是原始 markdown，并带有安全的链接/图片渲染。正文下方包括：
...
ArXiv 链接会通过 `llm_wiki/site/markdown.py` 自动链接；右侧栏 TOC 反映上面的 section 列表。
...
Syntheses 是由 `SynthesisProjector` 生成的高阶页面。七种类型（pulse, daily_digest, weekly, topic, comparison, field_overview）覆盖语料的时间和结构切面。当前 synthesis 正文是确定性模板；`LLM_WIKI_SYNTHESIS_LLM=1` 是 LLM 升级钩子（stub）。
...
Sessions 是导入的本地 AI-harness transcripts，会被规范化到 `.llm-wiki/harness_sessions/`，再渲染为可搜索的项目记忆。导入需要通过 `llm_wiki project sessions discover --import` 或 `llm_wiki project sessions import ...` 显式执行；普通站点构建只消费已经规范化的记录。
...
每日详情页 — 列出与该日历日相关的所有 paper / repo / concept / synthesis — 是明确的后续工作。Subagent P 正在通过 `StaticSiteBuilder` 接入每日详情输出。在完成之前，heatmap cells 会临时链接到当天的 `digest.md` source page。（内联 TODO 见 `llm_wiki/site/pages.py` 中的 `render_timeline`。）
...
页面内嵌 payload 上限为 `MAX_GRAPH_NODES = 1500`（见 [`pages.py`](../../llm_wiki/site/pages.py)）。完整 graph 始终可在 `/graph.json` 获取，供 tooling 使用。Code-graph nodes（`CodeClass`、`CodeFunction`、`Dependency`、…）按设计从可视化中滤除。
...
LLM-Wiki 以三种形式发布每个页面：面向人的 HTML、纯文本同级文件、结构化 JSON 同级文件。另有面向 crawlers 和 agents 的站点级导出。
...
- [Self-dogfood demo](self-dogfood.zh.md) — 对 LLM-Wiki 自身 repo 运行 LLM-Wiki，包括这个站点。
