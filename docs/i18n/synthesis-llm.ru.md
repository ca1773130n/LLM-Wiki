...
Полный блок занимает около 500 tokens. Канонический текст см. в [`tesserae/llm_synthesis.py`](../../tesserae/llm_synthesis.py). Любое изменение byte там инвалидирует prompt cache для каждой последующей страницы в запуске, поэтому rule text намеренно заморожен.
...
1. `TESSERAE_SYNTHESIS_LLM` равен `1`/`true`/`yes`/`on`.
2. `ANTHROPIC_API_KEY` не пустой (или вы задали `synthesis.api_key` в `.tesserae/config.json` проекта).
3. Package `anthropic` можно импортировать.

Если чего-то не хватает, в stderr пишется одна информационная строка (`[tesserae] LLM synthesis disabled (...)`), и происходит fallback к эвристике.
...
Если переключиться на Haiku 4.5 (`$1/M` input, `$5/M` output), тот же compile стоит примерно `~$0.027`. Сначала запустите с `TESSERAE_SYNTHESIS_DRY_RUN=1`, если хотите проверить форму prompt без расхода tokens.
...
Append-only ledger `.history.jsonl` в `.tesserae/wiki/syntheses/` записывает generator label для каждого rewrite, поэтому можно audit-ить, когда страница перешла с эвристики на LLM (или обратно).
