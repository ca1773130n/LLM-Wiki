def test_default_raganything_backend_config_has_required_fields():
    from tesserae.project import default_raganything_backend_config

    cfg = default_raganything_backend_config("demo")
    assert cfg["enabled"] is False
    assert cfg["working_dir"] == ".tesserae/external/raganything/working_dir"
    ...
    assert "raganything" in cfg["install"]["command"]
    assert "docling" in cfg["install"]["command"]
