...
from llm_wiki.understand_anything_refresh import refresh_understand_anything
...
def test_managed_refresh_skips_when_existing_graph_matches_git_head(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("llm_wiki.understand_anything_refresh._git_head", lambda root: "abc123")
...
def test_managed_refresh_runs_agent_cli_and_requires_artifact(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("llm_wiki.understand_anything_refresh.build_agent_command", fake_build)
    monkeypatch.setattr("llm_wiki.understand_anything_refresh.subprocess.run", fake_run)
